...
- **Result**: HTML interface reduced from 3.1MB → 22KB

### 🔍 **Real Corpus Processing**
- **Input**: `tests/real_corpus/metadata.json` (extracted filesystem metadata)
- **Processing**: Full Resonance workflow (scan → resolve → plan → apply)
...
- ❌ **No batch caps**: No `MAX_BATCHES`, no `batches[:5]`, no partial execution
- ❌ **No subset knobs**: Environment variables like `REAL_CORPUS_MAX_BATCHES` cause hard failure
- ✅ **Always complete**: If `RUN_REAL_CORPUS=1`, process ALL directories in metadata.json
- ✅ **CI separation**: Use `tests/fixture_corpus/` for fast CI validation
...
### **Decision 1: Process the Corpus (Authoritative)**
Run the full Resonance pipeline with live provider APIs and record your decisions.
...
**Requirements:**
- Set `ACOUSTID_API_KEY` and `DISCOGS_TOKEN` environment variables
- Interactive prompting for unresolved directories
- Records decisions to `tests/real_corpus/prompt_replay.json`
- Regenerates all expected outputs and review artifacts

**What it does:**
- Runs scan → resolve (with real APIs) → interactive prompt → plan → apply
- Records your choices for deterministic replay
- Updates expected_*.json files and review interface
- **Fails hard** without API credentials

### **Decision 2: Review Results (Read-Only)**
Inspect the processed corpus and make human canonicalness judgments.
...
**What it does:**
- Serves the review interface at http://localhost:8080/real_corpus_review.html
- No modifications to files or decisions
- Purely observational - browse, filter, and classify directories

---

## Advanced Options (Rarely Needed)

For testing or deterministic reproduction without network calls:

```bash
make corpus-decide-offline  # Uses recorded decisions, no network
```

This requires `tests/real_corpus/prompt_replay.json` from a prior `make corpus-decide` run.
...
- **Indicators**: Track counts, state badges (APPLIED ⚡, JAILED 🚫, PENDING ⏳)
...
- ❌ **Never** paste/load multi-MB JSON/HTML into LLM context
- ❌ **Never** open entire large files in editor pane
- ✅ **Only** inspect via: `ls -lh`, `head -n`, `jq` queries, small Python snippets
- ✅ **Generate** small summaries (<200KB) instead of reading full files
...
resonance/
├── scripts/
│   ├── generate_review_bundle.py      # Creates review_bundle.json
│   ├── generate_review_interface.py   # Creates HTML interface (Strategy A)
│   └── corpus_summary.py              # Safe summary (<200KB output)
├── tests/
│   ├── real_corpus/                   # Full real corpus (slow, authoritative)
│   │   ├── metadata.json              # Extracted filesystem data (1.3MB)
│   │   ├── expected_state.json        # Directory states
│   │   ├── expected_layout.json       # File organization
│   │   ├── expected_tags.json         # Expected metadata
│   │   └── decisions.json             # Scripted prompt decisions
│   └── fixture_corpus/               # CI fixture corpus (fast, committed)
│       ├── metadata.json              # Minimal test data (12KB)
│       ├── expected_state.json        # Expected states
│       ├── expected_layout.json       # Expected layout
│       └── expected_tags.json         # Expected tags
├── dist/                             # Generated interface assets
│   ├── real_corpus_review.html       # Main interface (22KB)
│   ├── index.json                    # Directory tree index (285KB)
│   ├── dir/*.json                    # Per-directory chunks (20 × ~5KB)
│   └── manifest.json                 # Audit hashes (38KB)
└── review_bundle.json               # Comprehensive review data (2.9MB)
```
...
### 🔄 **Workflow Status**
- **Corpus Extraction**: ✅ Automated via scripts
- **Processing Pipeline**: ✅ Full Resonance workflow with decision replay
- **Review Interface**: ✅ Strategy A implementation
- **Stability**: ✅ Zero context overflow risk
- **Auditability**: ✅ Complete hash chains and cryptographic validation
- **Decision Replay**: ✅ Semantically proven with hard enforcement

### 🎯 **Key Achievements**
- **20 directories** processed deterministically (full-corpus policy)
- **20 decisions** recorded with cryptographic fingerprints
- **HTML size**: 3.1MB → 22KB (99% reduction)
- **Agent safety**: No multi-MB files in context
- **Manual review**: Full directory tree navigation
- **Replay enforcement**: Hard failure on fingerprint mismatch
